...
const fileFunction = {
	...
	getFileType(file) {
		...
		/**
		 * 解析文件标签字符串为标签数组
		 * @param {string} tagList 标签字符串，格式: tagId:tagName:tagColor|tagId:tagName:tagColor
		 * @returns {array} 标签对象数组 [{tagId, tagName, tagColor}]
		 */
		parseTags(tagList) {
			if (!tagList) return []
			return tagList.split('|').map(item => {
				const [tagId, tagName, tagColor] = item.split(':')
				return { tagId: Number(tagId), tagName, tagColor }
			})
		},
