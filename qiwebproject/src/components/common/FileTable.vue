			...
			>
				...
				<template slot-scope="scope">
					...
					>
						...
						></span>
							<span class="file-tags" v-if="scope.row.tagList">
								<span
									v-for="tag in $file.parseTags(scope.row.tagList).slice(0, 2)"
									:key="tag.tagId"
									class="file-tag-dot"
								...
								></span>
								<span
									v-if="$file.parseTags(scope.row.tagList).length > 2"
									class="file-tag-more"
								>+{{ $file.parseTags(scope.row.tagList).length - 2 }}</span>
							</span>
