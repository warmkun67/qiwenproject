...
export default {
	...
	computed: {
		...
		// 按标签筛选的标签id
		tagId() {
			return this.$route.query.tagId ? Number(this.$route.query.tagId) : null
		},
	...
	methods: {
		 ...
		 * 表格数据获取相关事件 | 获取收藏文件列表
		 */
		showFavoriteFileList() {
			let data = {
				currentPage: this.pageData.currentPage,
				pageCount: this.pageData.pageCount
			}
			getFavoriteFileList(data).then((res) => {
				if (res.success) {
					this.fileList = res.dataList
					this.pageData.total = Number(res.total)
					this.loading = false
		...
		showFileListByTag() {
			...
			getFileListByTag(data).then((res) => {
				if (res.success) {
					this.fileList = res.dataList
					this.pageData.total = Number(res.total)
					this.loading = false
