<template>
	...
	<transition name="el-fade-in-linear">
		...
		>
			...
			<li
				class="right-menu-item"
				@click="handleToggleFavorite(selectedFile)"
				v-if="favoriteBtnShow"
			>
				<i :class="selectedFile.isFavorite === 1 ? 'el-icon-star-on' : 'el-icon-star-off'"></i>
				{{ selectedFile.isFavorite === 1 ? '取消收藏' : '收藏' }}
			</li>
			<li
				class="right-menu-item"
				@click="handleEditTags(selectedFile)"
				v-if="tagBtnShow"
			>
				<i class="el-icon-price-tag"></i> 编辑标签
			</li>
		</ul>
...
import { toggleFavorite } from '_r/file.js'
...
export default {
	...
	methods: {
		 ...
		 * @param {object} fileInfo 文件信息
		 */
		handleToggleFavorite(fileInfo) {
			this.visible = false
			toggleFavorite({ userFileId: fileInfo.userFileId }).then((res) => {
				if (res.success) {
		 ...
		 * @param {object} fileInfo 文件信息
		 */
		handleEditTags(fileInfo) {
			this.visible = false
			this.$openDialog.tagEdit({
				userFileId: fileInfo.userFileId
			}).then((res) => {
				if (res === 'confirm') {
					this.callback('confirm')
