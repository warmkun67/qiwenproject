...
public class UserFileService extends ServiceImpl<UserFileMapper, UserFile> implements IUserFileService {
    ...
    @Override
    public void toggleFavorite(String userFileId, String userId) {
        UserFile userFile = userFileMapper.selectById(userFileId);
        if (userFile == null || !userFile.getUserId().equals(userId)) {
            return;
        }
        Integer newFavorite = (userFile.getIsFavorite() != null && userFile.getIsFavorite() == 1) ? 0 : 1;
        LambdaUpdateWrapper<UserFile> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.set(UserFile::getIsFavorite, newFavorite)
                .eq(UserFile::getUserFileId, userFileId);
        userFileMapper.update(null, updateWrapper);
    }

    @Override
    public IPage<FileListVO> getFavoriteFileList(String userId, Long currentPage, Long pageCount) {
        Page<FileListVO> page = new Page<>(currentPage, pageCount);
        UserFile userFile = new UserFile();
        userFile.setUserId(userId);
        return userFileMapper.selectFavoritePageVo(page, userFile);
    }
