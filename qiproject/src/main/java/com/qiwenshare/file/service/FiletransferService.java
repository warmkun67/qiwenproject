        ...
        if (list != null && !list.isEmpty()) {
            ...
            try {
                ...
                if (userFile1 != null) {
                    ...
                    // 文件内容不同则重命名后重试插入
                    if (file1 != null && !StringUtils.equals(uploadFileDTO.getIdentifier(), file1.getIdentifier())) {
                        log.warn("极速上传文件冲突重命名处理: {}", JSON.toJSONString(userFile1));
                    ...
                    log.error("极速上传插入userFile失败，无重复记录: {}", e.getMessage());
        ...
        for (int i = 0; i < uploadFileResultList.size(); i++){
            ...
            if (UploadFileStatusEnum.SUCCESS.equals(uploadFileResult.getStatus())){
                ...
                try {
                    ...
                    if (userFile1 != null) {
                        FileBean file1 = fileMapper.selectById(userFile1.getFileId());
                        if (file1 != null && !StringUtils.equals(fileBean.getIdentifier(), file1.getIdentifier())) {
                        ...
                        log.error("上传文件插入userFile失败，无重复记录: {}", e.getMessage());
                        throw new RuntimeException("文件上传失败", e);
                    }
                }
