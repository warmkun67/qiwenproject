...
public class FileController {
    ...
    @Operation(summary = "收藏/取消收藏文件", description = "切换文件的收藏状态", tags = {"file"})
    @MyLog(operation = "切换收藏", module = CURRENT_MODULE)
    @RequestMapping(value = "/favorite", method = RequestMethod.POST)
    @ResponseBody
    public RestResult<String> toggleFavorite(@Valid @RequestBody FavoriteFileDTO favoriteFileDTO) {
        JwtUser sessionUserBean = SessionUtil.getSession();
        userFileService.toggleFavorite(favoriteFileDTO.getUserFileId(), sessionUserBean.getUserId());
        return RestResult.success().message("操作成功");
    }

    @Operation(summary = "获取收藏文件列表", description = "获取当前用户收藏的文件列表", tags = {"file"})
    @RequestMapping(value = "/getfavoritefilelist", method = RequestMethod.GET)
    @ResponseBody
    public RestResult<FileListVO> getFavoriteFileList(
            @Parameter(description = "当前页", required = true) long currentPage,
            @Parameter(description = "页面数量", required = true) long pageCount) {
        JwtUser sessionUserBean = SessionUtil.getSession();
        IPage<FileListVO> fileList = userFileService.getFavoriteFileList(sessionUserBean.getUserId(), currentPage, pageCount);
        return RestResult.success().dataList(fileList.getRecords(), fileList.getTotal());
    }

    @Operation(summary = "按标签获取文件列表", description = "根据标签id获取文件列表", tags = {"file"})
    @RequestMapping(value = "/getfilelistbytag", method = RequestMethod.GET)
    @ResponseBody
    public RestResult<FileListVO> getFileListByTag(
            @Parameter(description = "标签id", required = true) Long tagId,
            @Parameter(description = "当前页", required = true) long currentPage,
            @Parameter(description = "页面数量", required = true) long pageCount) {
        JwtUser sessionUserBean = SessionUtil.getSession();
        ...
        IPage<FileListVO> fileList = userFileMapper.selectPageVoByTag(page, userFile, tagId);
        return RestResult.success().dataList(fileList.getRecords(), fileList.getTotal());
    }
